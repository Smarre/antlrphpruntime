...
abstract class BaseRecognizer
{
    ...
    public function getToken($name)
    {
        if (is_numeric($name)) {
            return (integer) $name;
        } else if (defined(get_class($this). "::T_" .$name)) {
            return constant(get_class($this) . "::T_" . $name);
        } else {
            // TODO: Make tokens constants and re-use them from the lexer
            return $this->$name;
        }
    }
    ...
    function expand_u_escape($string) {
        $va = intval(substr($string, 2), 16 );
        return $va;
    }
	...
	public function castToInt($value){
        ...
        {
            return (integer) $this->expand_u_escape($value);
        }
        if ($value == '\\')
        {
            $value = ord($value);
		...
		$replaced = $this->expand_escape($value);
		$ret = ord($replaced);
		return $ret;
	}
