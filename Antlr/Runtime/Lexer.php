...
abstract class Lexer extends BaseRecognizer
{
    ...
    {
        if ($token == null) {
            $token = new CommonToken($this->input, $this->state->type, $this->state->channel,
    ...
    {
        if ($this->input->LA(1) != $c) { // KED $this->castToInt($c)) {
            if ($this->state->backtracking > 0) {
                $this->state->failed = true;
                return;
            }
    ...
    {
        if ($this->input->LA(1) < $a || $this->input->LA(1) > $b) { // KED $this->castToInt($a) || $this->input->LA(1) > $this->castToInt($b)) {
            if ($this->state->backtracking > 0) {
                $this->state->failed = true;
                return;
            }
    ...
    public function getErrorMessage($e, $tokenNames)
    {
        ...
        if ($e instanceof MismatchedTokenException) {
            ...
            $msg = "mismatched character " . $this->getCharErrorDisplay($e->c) .
                    " expecting " . $this->getCharErrorDisplay($mte->expecting); // KED ord($mte->expecting)); //; $this->getCharErrorDisplay($mte->expecting);
